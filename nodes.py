...
def encode_with_chunking_enhanced(encoder, prompts, device_to, max_tokens, chunk_by_double_line=False):
    """Enhanced encode prompts with chunking support, including double newline splitting"""
    results = []
    tokenizer = get_t5_tokenizer(encoder)
    ...
    for prompt in prompts:
        ...
        if not chunk_by_double_line and token_count <= max_tokens:
            # Short enough and not using double line chunking - encode normally
            encoded = encoder([prompt], device_to)
            results.append(encoded[0])
        else:
            ...
            if chunk_by_double_line:
                # First split by double newlines
                log.info(f"Splitting prompt by double newlines first")
                initial_chunks = split_text_by_double_newline(prompt)
                log.info(f"Initial double-newline split created {len(initial_chunks)} chunks")
                
                # Then check each chunk for token limits and split further if needed
                for i, chunk in enumerate(initial_chunks):
                    chunk_token_count = get_token_count(encoder, chunk)
                    log.info(f"Double-newline chunk {i+1}: {chunk_token_count} tokens")
                    
                    if chunk_token_count <= max_tokens:
                        chunks.append(chunk)
                    else:
                        # Split this chunk further by sentences
                        log.info(f"Double-newline chunk {i+1} exceeds {max_tokens} tokens, splitting by sentences")
                        sentence_chunks = split_text_by_sentences(chunk, max_tokens)
                        chunks.extend(sentence_chunks)
                        log.info(f"Split into {len(sentence_chunks)} sentence chunks")
            else:
                # Use the original sentence-based chunking
                log.info(f"Chunking prompt: {token_count} tokens -> chunks of {max_tokens}")
                chunks = split_text_by_sentences(prompt, max_tokens)
    ...
    def INPUT_TYPES(s):
        return {"required": {
            ...
            "optional": {
                ...
                "chunk_by_double_line": ("BOOLEAN", {"default": False, "tooltip": "When enabled with chunking, always split text by double newlines first, then by token limits"}),
            }
        }
...
"""
    ...
    def process(self, model_name, precision, positive_prompt, negative_prompt, quantization='disabled', use_disk_cache=True, device="gpu", extender_args=None,
                max_chunk_tokens=480, enable_chunking=True, chunk_by_double_line=False):
        ...
        prompt_embeds_dict, = WanVideoTextEncode().process(
            ...
            chunk_by_double_line=chunk_by_double_line
        )
        pbar.update(1)
        del t5
        mm.soft_empty_cache()
...
class WanVideoTextEncode:
    ...
    def process(self, positive_prompt, negative_prompt, t5=None, force_offload=True, model_to_offload=None, use_disk_cache=False, device="gpu", max_chunk_tokens=480, enable_chunking=True, chunk_by_double_line=False):
        ...
        with torch.autocast(device_type=mm.get_autocast_device(device_to), dtype=encoder.dtype, enabled=encoder.quantization != 'disabled'):
            ...
            if use_disk_cache and context is not None:
                ...
                if enable_chunking and (chunk_by_double_line or needs_chunking(encoder, positive_prompts, max_chunk_tokens)):
                    if chunk_by_double_line:
                        log.info("Using double newline chunking for positive prompts")
                    else:
                        log.info("Long prompts detected, using chunked encoding")
                    context = encode_with_chunking_enhanced(encoder, positive_prompts, device_to, max_chunk_tokens, chunk_by_double_line)
                else:
            ...
            if use_disk_cache and context_null is not None:
                ...
                if enable_chunking and (chunk_by_double_line or get_token_count(encoder, negative_prompt) > max_chunk_tokens):
                    if chunk_by_double_line:
                        log.info("Using double newline chunking for negative prompt")
                    else:
                        log.info("Long negative prompt detected, using chunked encoding")
                    context_null = encode_with_chunking_enhanced(encoder, [negative_prompt], device_to, max_chunk_tokens, chunk_by_double_line)
                else:
...
NODE_CLASS_MAPPINGS = {
    "VTSWanVideoSampler": WanVideoSampler,
    "VTSWanVideoLoopingSampler": WanVideoLoopingSampler,
    "VTSWanVideoDecode": WanVideoDecode,
    "VTSWanVideoTextEncode": WanVideoTextEncode,
    "VTSWanVideoTextEncodeSingle": WanVideoTextEncodeSingle,
    "VTSWanVideoClipVisionEncode": WanVideoClipVisionEncode,
    "VTSWanVideoImageToVideoEncode": WanVideoImageToVideoEncode,
    "VTSWanVideoImageSequenceEncode": WanVideoImageSequenceEncode,
    "VTSWanVideoEncode": WanVideoEncode,
    "VTSWanVideoEncodeLatentBatch": WanVideoEncodeLatentBatch,
    "VTSWanFaceRestoreArgs": WanFaceRestoreArgs_VTS,
    "VTSWanColorMatchArgs": WanColorMatchArgs_VTS,
    "VTSWanVideoDecodeSettings": WanVideoDecodeSettings_VTS,
    "VTSWanVideoEncodeSettings": WanVideoEncodeSettings_VTS,
    "VTSWanVideoUpscaleWithModelSettings": WanVideoUpscaleWithModelSettings_VTS,
    "VTSWanVideoSimpleScaleSettings": WanVideoSimpleScaleSettings_VTS,
    "VTSWanVideoEmptyEmbeds": WanVideoEmptyEmbeds,
    "VTSWanVideoEnhanceAVideo": WanVideoEnhanceAVideo,
    "VTSWanVideoContextOptions": WanVideoContextOptions,
    "VTSWanVideoTextEmbedBridge": WanVideoTextEmbedBridge,
    "VTSWanVideoTextEmbedReverseBridge": WanVideoTextEmbedReverseBridge,
    "VTSWanVideoLoopingControlImagesOptions": WanVideoLoopingControlImagesOptions,
    "VTSWanVideoFlowEdit": WanVideoFlowEdit,
    "VTSWanVideoControlEmbeds": WanVideoControlEmbeds,
    "VTSWanVideoSLG": WanVideoSLG,
    "VTSWanVideoLoopArgs": WanVideoLoopArgs,
    "VTSWanVideoSetBlockSwap": WanVideoSetBlockSwap,
    "VTSWanVideoExperimentalArgs": WanVideoExperimentalArgs,
    "VTSWanVideoVACEEncode": WanVideoVACEEncode,
    "VTSWanVideoPhantomEmbeds": WanVideoPhantomEmbeds,
    "VTSWanVideoRealisDanceLatents": WanVideoRealisDanceLatents,
    "VTSWanVideoApplyNAG": WanVideoApplyNAG,
    "VTSWanVideoApplyMultiNAG": VTS_WanVideoApplyMultiNAG,
    "VTSWanVideoMiniMaxRemoverEmbeds": WanVideoMiniMaxRemoverEmbeds,
    "VTSWanVideoFreeInitArgs": WanVideoFreeInitArgs,
    "VTSWanVideoSetRadialAttention": WanVideoSetRadialAttention,
    "VTSWanVideoBlockList": WanVideoBlockList,
    "VTSWanVideoTextEncodeCached": WanVideoTextEncodeCached,
    "VTSWanVideoMultiTextEncodeCached": VTS_WanVideoMultiTextEncodeCached,
    "VTSWanT5Comparer": VTS_WanT5Comparer,
    "VTSClipComparer": VTS_ClipComparer,
    "VTSWanVideoAddExtraLatent": WanVideoAddExtraLatent,
    "VTSWanVideoScheduler": WanVideoScheduler,
    "VTSWanVideoAddStandInLatent": WanVideoAddStandInLatent,
    "VTSWanVideoAddControlEmbeds": WanVideoAddControlEmbeds,
    "VTSWanVideoAddMTVMotion": WanVideoAddMTVMotion,
    "VTSWanVideoRoPEFunction": WanVideoRoPEFunction,
    "VTSWanVideoAddPusaNoise": WanVideoAddPusaNoise
    }
NODE_DISPLAY_NAME_MAPPINGS = {
    "VTSWanVideoSampler": "VTS WanVideo Sampler",
    "VTSWanVideoLoopingSampler": "VTS WanVideo Looping Sampler",
    "VTSWanVideoDecode": "VTS WanVideo Decode",
    "VTSWanVideoTextEncode": "VTS WanVideo TextEncode",
    "VTSWanVideoTextEncodeSingle": "VTS WanVideo TextEncodeSingle",
    "VTSWanVideoTextImageEncode": "VTS WanVideo TextImageEncode (IP2V)",
    "VTSWanVideoClipVisionEncode": "VTS WanVideo ClipVision Encode",
    "VTSWanVideoImageToVideoEncode": "VTS WanVideo ImageToVideo Encode",
    "VTSWanVideoImageSequenceEncode": "VTS WanVideo ImageSequence Encode",
    "VTSWanVideoEncode": "VTS WanVideo Encode",
    "VTSWanVideoEncodeLatentBatch": "VTS WanVideo Encode Latent Batch",
    "VTSWanFaceRestoreArgs": "VTS WanFace Restore Args",
    "VTSWanColorMatchArgs": "VTS WanColor Match Args",
    "VTSWanVideoDecodeSettings": "VTS WanVideo Decode Settings",
    "VTSWanVideoEncodeSettings": "VTS WanVideo Encode Settings",
    "VTSWanVideoUpscaleWithModelSettings": "VTS WanVideo Upscale With Model Settings",
    "VTSWanVideoSimpleScaleSettings": "VTS WanVideo Simple Scale Settings",
    "VTSWanVideoEmptyEmbeds": "VTS WanVideo Empty Embeds",
    "VTSWanVideoEnhanceAVideo": "VTS WanVideo Enhance-A-Video",
    "VTSWanVideoContextOptions": "VTS WanVideo Context Options",
    "VTSWanVideoTextEmbedBridge": "VTS WanVideo TextEmbed Bridge",
    "VTSWanVideoTextEmbedReverseBridge": "VTS WanVideo TextEmbed Reverse Bridge",
    "VTSWanVideoLoopingControlImagesOptions": "VTS WanVideo Looping Control Images Options",
    "VTSWanVideoFlowEdit": "VTS WanVideo FlowEdit",
    "VTSWanVideoControlEmbeds": "VTS WanVideo Control Embeds",
    "VTSWanVideoSLG": "VTS WanVideo SLG",
    "VTSWanVideoLoopArgs": "VTS WanVideo Loop Args",
    "VTSWanVideoSetBlockSwap": "VTS WanVideo Set BlockSwap",
    "VTSWanVideoExperimentalArgs": "VTS WanVideo Experimental Args",
    "VTSWanVideoVACEEncode": "VTS WanVideo VACE Encode",
    "VTSWanVideoPhantomEmbeds": "VTS WanVideo Phantom Embeds",
    "VTSWanVideoRealisDanceLatents": "VTS WanVideo RealisDance Latents",
    "VTSWanVideoApplyNAG": "VTS WanVideo Apply NAG",
    "VTSWanVideoApplyMultiNAG": "VTS WanVideo Apply Multi NAG",
    "VTSWanVideoMiniMaxRemoverEmbeds": "VTS WanVideo MiniMax Remover Embeds",
    "VTSWanVideoFreeInitArgs": "VTS WanVideo Free Init Args",
    "VTSWanVideoSetRadialAttention": "VTS WanVideo Set Radial Attention",
    "VTSWanVideoBlockList": "VTS WanVideo Block List",
    "VTSWanVideoTextEncodeCached": "VTS WanVideo TextEncode Cached",
    "VTSWanVideoMultiTextEncodeCached": "VTS WanVideo MultiTextEncode Cached",
    "VTSWanT5Comparer": "VTS WanT5 Comparer",
    "VTSClipComparer": "VTS Clip Comparer",
    "VTSWanVideoAddExtraLatent": "VTS WanVideo Add Extra Latent",
    "VTSWanVideoAddStandInLatent": "VTS WanVideo Add StandIn Latent",
    "VTSWanVideoAddControlEmbeds": "VTS WanVideo Add Control Embeds",
    "VTSWanVideoAddMTVMotion": "VTS WanVideo MTV Crafter Motion",
    "VTSWanVideoRoPEFunction": "VTS WanVideo RoPE Function",
    "VTSWanVideoAddPusaNoise": "VTS WanVideo Add Pusa Noise",
}
