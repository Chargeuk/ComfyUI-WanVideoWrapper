...
class WanVideoVACEModelSelect:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "vace_model": (get_filename_list("unet_gguf") + get_filename_list("diffusion_models"), {"tooltip": "These models are loaded from the 'ComfyUI/models/diffusion_models' VACE model to use when not using model that has it included"}),
            },
        }
...
class WanVideoModelLoader:
    ...
    def loadmodel(self, model, base_precision, load_device,  quantization,
        ...
        if lora is not None:
            lorasWithStrength = False
            # first check if there are any Loras with a strength greater than 0
            for l in lora:
                # Exclude LoRAs with strength 0
                if l["strength"] > 0:
                    lorasWithStrength = True
                    continue
    ...
    def savemodel(self, model, filename, output_path=""):
        ...
        try:
            ...
            metadata = {
                ...
                "base_dtype": str(model.model["base_dtype"]),
...
class WanVideoLoraMerger:
    ...
    def merge_loras(self, output_name, lora=None):
        ...
        for l in lora:
            ...
            # Convert fp8 weights to target dtype to avoid CPU operation errors
            lora_sd = self.convert_fp8_weights(lora_sd, torch.bfloat16)
    ...
    def convert_fp8_weights(self, lora_sd, target_dtype):
        """Convert fp8 weights to target dtype to avoid CPU operation errors"""
        import torch
        converted_sd = {}
        fp8_detected = False
        ...
        for key, value in lora_sd.items():
            if isinstance(value, torch.Tensor):
                # Check for fp8 dtypes and convert them
                if value.dtype in [torch.float8_e4m3fn, torch.float8_e5m2]:
                    if not fp8_detected:
                        log.info(f"FP8 weights detected in LoRA, converting to {target_dtype}")
                        fp8_detected = True
                    # Convert fp8 to target dtype
                    converted_sd[key] = value.to(target_dtype)
                else:
                    # Keep non-fp8 tensors as-is, but ensure they're compatible dtype
                    if value.dtype in [torch.float32, torch.float16, torch.bfloat16]:
                        converted_sd[key] = value.to(target_dtype)
                    else:
                        # For integer types (alpha values), keep as-is
                        converted_sd[key] = value
            else:
                # Non-tensor values (shouldn't happen in LoRAs, but just in case)
                converted_sd[key] = value
...
NODE_CLASS_MAPPINGS = {
    "VTSWanVideoModelLoader": WanVideoModelLoader,
    "VTSWanVideoLoraMerger": WanVideoLoraMerger,
    "VTSWanVideoSaveModel": WanVideoSaveModel,
    "VTSWanVideoVAELoader": WanVideoVAELoader,
    "VTSWanVideoLoraSelect": WanVideoLoraSelect,
    "VTSWanVideoLoraSelectByName": WanVideoLoraSelectByName,
    "VTSWanVideoSetLoRAs": WanVideoSetLoRAs,
    "VTSWanVideoLoraBlockEdit": WanVideoLoraBlockEdit,
    "VTSWanVideoTinyVAELoader": WanVideoTinyVAELoader,
    "VTSWanVideoVACEModelSelect": WanVideoVACEModelSelect,
    "VTSWanVideoExtraModelSelect": WanVideoExtraModelSelect,
    "VTSWanVideoLoraSelectMulti": WanVideoLoraSelectMulti,
    "VTSWanVideoBlockSwap": WanVideoBlockSwap,
    "VTSWanVideoVRAMManagement": WanVideoVRAMManagement,
    "VTSWanVideoTorchCompileSettings": WanVideoTorchCompileSettings,
    "VTSLoadWanVideoT5TextEncoder": LoadWanVideoT5TextEncoder,
    "VTSLoadWanVideoClipTextEncoder": LoadWanVideoClipTextEncoder,
    }

NODE_DISPLAY_NAME_MAPPINGS = {
    "VTSWanVideoModelLoader": "VTS WanVideo Model Loader",
    "VTSWanVideoLoraMerger": "VTS WanVideo Lora Merger",
    "VTSWanVideoSaveModel": "VTS WanVideo Save Model",
    "VTSWanVideoVAELoader": "VTS WanVideo VAE Loader",
    "VTSWanVideoLoraSelect": "VTS WanVideo Lora Select",
    "VTSWanVideoLoraSelectByName": "VTS WanVideo Lora Select By Name",
    "VTSWanVideoSetLoRAs": "VTS WanVideo Set LoRAs",
    "VTSWanVideoLoraBlockEdit": "VTS WanVideo Lora Block Edit",
    "VTSWanVideoTinyVAELoader": "VTS WanVideo Tiny VAE Loader",
    "VTSWanVideoVACEModelSelect": "VTS WanVideo VACE Module Select",
    "VTSWanVideoExtraModelSelect": "VTS WanVideo Extra Model Select",
    "VTSWanVideoLoraSelectMulti": "VTS WanVideo Lora Select Multi",
    "VTSWanVideoBlockSwap": "VTS WanVideo Block Swap",
    "VTSWanVideoVRAMManagement": "VTS WanVideo VRAM Management",
    "VTSWanVideoTorchCompileSettings": "VTS WanVideo Torch Compile Settings",
    "VTSLoadWanVideoT5TextEncoder": "VTS WanVideo T5 Text Encoder Loader",
    "VTSLoadWanVideoClipTextEncoder": "VTS WanVideo CLIP Text Encoder Loader",
    }